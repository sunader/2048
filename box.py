...
class GameInit(object):
    ...
    def drawSurface(cls, screen, matrix, score):
        ...
        screen.blit(font2.render('left  down  right',
                                 True, (255, 127, 0)), (300, 50))
        a, b = matrix.shape
        for i in range(a):
            for j in range(b):
                cls.drawBlock(
                    screen, i, j, Block_Color[matrix[i][j]], matrix[i][j])
    ...
    def drawBlock(screen, row, column, color, blocknum):
        ...
        if blocknum != 0:
            ...
            screen.blit(font.render(str(int(blocknum)), True, (0, 0, 0)),
                        (w + (110 - fw) / 2, h + (110 - fh) / 2))
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                actionObject = GameInit.keyDownPressed(
                    event.key, matrix)  # 创建各种动作类的对象
